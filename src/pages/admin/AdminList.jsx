...
import { AlertCheck } from "../../utils/alertAdminCheck";
      ...
      <div className="sticky top-0 flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-5 mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-center gap-2 sm:gap-3">
          <button onClick={() => {
            ...
            className={`${role !== "admin" ? "opacity-50" : ""} bg-blue-500 hover:bg-blue-600 transition-colors w-full sm:w-auto px-6 py-2.5 sm:py-3 text-white rounded-xl font-medium cursor-pointer text-sm sm:text-base`}>
            {t("add_user")}
          </button>
          {/* Excel Export */}
        ...
        {FilteredUser.filter((item) => item.role === "admin").sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item, index) => (
          ...
          >
            ...
            <div className="text-center flex justify-center items-center gap-4">
              <Edit
                className={`cursor-pointer ${role !== "admin" ? "opacity-50 " : ""}`}
                onClick={(e) => {
                  e.stopPropagation();
                  if (role !== "admin") {
                    AlertCheck(role);
              ...
              <Trash
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (role !== "admin") {
                    AlertCheck(role, "can do delete user");
                    return;
                  }
